...
import { CreateFood, MenuCategory } from "@ts-types/generated";
...
import { useCreateFoodMutation } from "@data/products/create-food-mutation";
...
const initialValues: FormValues = {
    name: "",
    menuCategory: null,
    productType: { label: FoodProductTypes.simple, value: FoodProductTypes.simple },
    price: null,
    salePrice: null,
    quantity: null,
    description: "",
    images: [],
    requiredFields: [initialRequiredFieldVariationValue],
    optionalFields: [initialOptionalFieldVariationValue],
    minPrice: null,
    maxPrice: null
}
...
const CreateFoodForm = () => {
    ...
    const { mutate, isLoading } = useCreateFoodMutation()

    // fetching Shop Categories 
    const { data: categories, isLoading: fetchingCategories } = useMenuCategoriesQuery({
        limit: 99999,
        page: 1,
    });
    ...
    const createProduct = (values: FormValues, resetForm: any) => {
        ...
        const foodProductValues: CreateFood = {
            ...values,
        ...
        if (values.productType.value === FoodProductTypes.variation) {
            ...
            if (values?.maxPrice === null || values?.maxPrice === undefined) {
                delete foodProductValues?.maxPrice;
            }
                    ...
                    <form noValidate autoComplete='off' onSubmit={handleSubmit}>
                        <Grid container>
                            ...
                            <Grid item xs={12} md={7} lg={8} >
                                ...
                                <CustomSelect
                                    name="menuCategory"
                                    list={categories?.categories.data.map((category: MenuCategory) => ({ label: category.title, value: category._id }))}
