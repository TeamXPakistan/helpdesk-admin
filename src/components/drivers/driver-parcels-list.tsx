...
const DriverParcelsList = ({ data, onPaginationChange, paginatorInfo }: PropTypes) => {
    ...
    const defaultColumns2: GridColDef[] = [
        ...
        },
        {
            ...
            renderCell: ({ row }: { row: Order }) => {
                return (<>
                    <Box>
                        <IconButton color='inherit' title='View Order' aria-haspopup='true' onClick={() => openModal({ view: "DRIVER_PARCEL_DETAILS_VIEW", data: { id: row?._id } })}>
...
export default DriverParcelsList;